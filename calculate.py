...
# 按钮操作，点击按钮后需要做的处理
def buttonclik(btn):
    content = contentshow.get()  # 获取文本框中的内容
    ...
    if btn in '0123456789':
        ...
        if all(map(lambda x: x.isdigit(), n)):  # 如果列表中所有的都是数字，就是为了检查表达式是不是正确的
            ...
            return
    contentshow.set(content)  # 将结果显示到文本框中
...
contentshow = tkinter.StringVar(root, '')
contentEntry = tkinter.Entry(root, textvariable=contentshow)  # 括号里面，可见第一个都是root,即表示都是以root为主界面的，将文本框中的内容存在contentshow中
...
digits = list('0123456789.') + ['Sqrt']  # 元祖序列list是Python中最基本的数据结构。序列中的每个元素都分配一个数字 - 它的位置，或索引，第一个索引是0，第二个索引是1，依此类推。
index = 0
